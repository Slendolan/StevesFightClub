...
The graphs indicate the total rewards of each episode. The x-axis represents the episode and the y-axis represents the reward of that episode. The graph on the right is based on the map generated in our initial approach. Observe that the reward increases as the number of episode increases. This is most likely due to the fact that the agent is taking Q table moves more than random moves as it approaches 200 episodes. On the other hand, the left graph is a newly generated map with different zombie spawn positions shown below:

<img src="img/challenge_spawn.png" width="250" height="250">

Once the zombie spawn positions changed, the rewards do not improve over the course of 200 episodes. The enemies are placed diagonally in front and behind the agent where both enemies are equidistant to the agent. The agent will have difficulty locking on to one enemy. This is confirmed by observing the agent over several episodes. It would spin repeatedly and switch between the two targets. This results in the agent occasionally missing attacks, taking damage, and consequently dying. 


## Remaining Goals and Challenges

Over the next few weeks, we plan to complete a few of these remaining goals
- Fight a wider variety of enemies
- A baseline to compare performance
- Use more weapons such as bows and arrows or shields
- Reduce the spamming of attacks
- Improve agent's doging capabilities
